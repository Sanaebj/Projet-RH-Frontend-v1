  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        console.log('Réunions brutes :', data);

        // Formater les réunions pour s'assurer que participations existe et nomComplet est défini
        const formatted = data.map(r => ({
          ...r,
          participations: (r.participations ?? []).map(p => ({
            employeId: p.employeId,
            nomComplet: p.nomComplet ?? 'Nom indisponible',
          })),
        }));

        formatted.forEach(r => {
          console.log(`Participants de la réunion ${r.id} :`, r.participations);
        });

        setReunions(formatted);
      } catch (error) {
        console.error('Erreur lors du fetch des réunions :', error);
        setReunions([]);
      }
    };
          ...
          <TableBody>
            {reunions.map(reunion => (
              <TableRow key={reunion.id}>
                ...
                <TableCell>
                  {(reunion.participations ?? []).length > 0 ? (
                    <ul style={{ paddingLeft: 16, margin: 0 }}>
                      {(reunion.participations ?? []).map(p => (
                        <li key={p.employeId}>{p.nomComplet}</li>
                      ))}
                    </ul>
                  ) : (
                    <em>Aucun</em>
