...
const ProfileEmployee = () => {
  ...
  // Charger l'utilisateur connecté + solde congés
  useEffect(() => {
    const fetchUser = async () => {
      try {
        ...
        // Décoder le token pour récupérer l'id employé
        const token = localStorage.getItem("token");
        if (token) {
          const decoded: TokenPayload = decodeToken(token);
          const employeId = Number(decoded.userId);

          // Récupération du solde congés pour l’employé
          const soldeRes = await axiosInstance.get(`/conges/solde/${employeId}`);
          const solde =
            typeof soldeRes.data === "number"
              ? soldeRes.data
              : soldeRes.data?.droitAnnuel ?? 0;
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (!formData.password) delete updateData.password; // éviter d'envoyer un mot de passe vide
      ...
      <Box mt={8}>
        <Card sx={{ borderRadius: 4, boxShadow: 6, p: 3 }}>
          <CardContent>
            ...
            {/* ✅ Solde de congés spécifique employé */}
            <Paper
              elevation={0}
              sx={{ p: 2, bgcolor: "grey.100", borderRadius: 2, mb: 3, textAlign: "center" }}
            >
            ...
            {/* Infos utilisateur avec édition */}
            <Paper
              elevation={0}
