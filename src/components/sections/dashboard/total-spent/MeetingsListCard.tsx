...
const MeetingsListCard = () => {
  ...
  useEffect(() => {
    ...
    try {
      ...
      // ✅ Charger les réunions seulement si rôle = ADMIN
      if (decodedRole.toUpperCase() === "ADMIN") {
        const fetchReunions = async () => {
          try {
            const response = await fetch(
              "http://localhost:2233/api/reunions/upcoming",
              {
                headers: {
            ...
            const monthlyData = data.reduce(
              (acc: Record<string, number>, reunion) => {
            ...
            reunions.map((meeting, index) => (
              <ListItem key={index} disablePadding>
                ...
                <ListItemText
                  ...
                  secondary={dayjs(meeting.dateHeure).format(
